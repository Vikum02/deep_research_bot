...
function Chat() {
  ...
  function parseInlineMarkdown(text) {
    ...
    while (i < current_text.length) {
      if (i < current_text.length - 5 && current_text[i] === "*" && current_text[i + 1] === "*" && current_text[i + 2] === "*") {
        if (buffer.length > 0) {
          segments.push({text: buffer, bold: false, italic: false, code: false});
          buffer = "";
        }
        let end = current_text.indexOf("***", i + 3);
        if (end !== -1) {
          let bold_italic_text = current_text.substring(i + 3, end);
          segments.push({text: bold_italic_text, bold: true, italic: true, code: false});
          i = end + 3;
          continue;
        }
        ...
        if (end !== -1) {
          let bold_text = current_text.substring(i + 2, end);
          segments.push({text: bold_text, bold: true, italic: false, code: false});
          i = end + 2;
          continue;
        }
        ...
        if (end < current_text.length) {
          let italic_text = current_text.substring(i + 1, end);
          segments.push({text: italic_text, bold: false, italic: true, code: false});
          i = end + 1;
          continue;
        }
        ...
        if (end !== -1) {
          let code_text = current_text.substring(i + 1, end);
          segments.push({text: code_text, bold: false, italic: false, code: true});
          i = end + 1;
          continue;
        }
      }
  ...
  function renderFormattedText(text) {
    ...
    return __jacJsx(Box, {"component": "span"}, [segments.map((seg, idx) => {
      let sx_style = {};
      if (seg.bold) {
      ...
      return __jacJsx(Typography, {"key": idx, "component": "span", "sx": sx_style}, [seg.text]);
    })]);
  }
  ...
  function renderMarkdownReport(msg) {
    ...
    return __jacJsx(Box, {}, [__jacJsx(Typography, {"variant": "h6", "sx": {mb: 2, fontWeight: 700, color: "primary.main"}}, ["🔬 Deep Research Report"]), __jacJsx(Typography, {"variant": "subtitle2", "sx": {mb: 3, color: "text.secondary", fontStyle: "italic"}}, ["Topic: ", topic]), __jacJsx(Box, {"sx": {mb: 3, p: 2, bgcolor: "primary.50", borderRadius: 2, borderLeft: "4px solid", borderColor: "primary.main"}}, [__jacJsx(Typography, {"variant": "subtitle1", "sx": {mb: 1.5, fontWeight: 700, color: "primary.dark"}}, ["📊 Executive Summary"]), __jacJsx(Typography, {"sx": {lineHeight: 1.8, color: "text.primary"}}, [renderFormattedText(executive_summary)])]), __jacJsx(Box, {"sx": {mb: 3, p: 2, bgcolor: "secondary.50", borderRadius: 2, borderLeft: "4px solid", borderColor: "secondary.main"}}, [__jacJsx(Typography, {"variant": "subtitle1", "sx": {mb: 1.5, fontWeight: 700, color: "secondary.dark"}}, ["🔍 Key Findings"]), key_findings.map((f, idx) => {
      return __jacJsx(Box, {"key": idx, "sx": {mb: 1.5, display: "flex", alignItems: "flex-start"}}, [__jacJsx(Typography, {"sx": {mr: 1, color: "secondary.main", fontWeight: 700, minWidth: "24px"}}, ["•"]), __jacJsx(Typography, {"sx": {flex: 1, lineHeight: 1.7}}, [renderFormattedText(f)])]);
    })]), content_sections.map((section, idx) => {
      return __jacJsx(Box, {"key": idx, "sx": {mb: 3, p: 2, bgcolor: "grey.50", borderRadius: 2, borderLeft: "4px solid", borderColor: "grey.400"}}, [__jacJsx(Typography, {"variant": "subtitle1", "sx": {mb: 1.5, fontWeight: 700, color: "text.primary"}}, ["📈 ", renderFormattedText(section.title)]), __jacJsx(Typography, {"sx": {whiteSpace: "pre-wrap", lineHeight: 1.8, color: "text.primary"}}, [renderFormattedText(section.content)])]);
    }), __jacJsx(Box, {"sx": {mt: 4, p: 2.5, bgcolor: "grey.100", borderRadius: 2, border: "2px dashed", borderColor: "grey.300"}}, [__jacJsx(Typography, {"variant": "subtitle2", "sx": {fontWeight: 700, mb: 1.5, color: "text.primary"}}, ["📚 Research Summary"]), __jacJsx(Box, {"sx": {display: "flex", flexDirection: "column", gap: 0.75}}, [__jacJsx(Typography, {"variant": "body2", "sx": {color: "text.secondary"}}, ["• Total searches conducted: ", __jacJsx("strong", {}, [search_count])]), __jacJsx(Typography, {"variant": "body2", "sx": {color: "text.secondary"}}, ["• Deep research with multiple sources per topic"]), __jacJsx(Typography, {"variant": "body2", "sx": {color: "text.secondary"}}, ["• Personalized to your specific needs"])])])]);
  }
  ...
  }, "placeholder": "Type your research topic here...", "variant": "outlined", "disabled": loading, "sx": {backgroundColor: "white", borderRadius: 2, transition: "all 0.3s ease", "& .MuiOutlinedInput-root": {"&:hover fieldset": {borderColor: "primary.main"}, "&.Mui-focused fieldset": {borderColor: "primary.main", borderWidth: "2px"}}, "&:hover": {boxShadow: "0 2px 8px rgba(99, 102, 241, 0.2)"}}}, []), __jacJsx(Button, {"variant": "contained", "color": "primary", "onClick": sendMessage, "disabled": loading || !currentMessage.trim(), "sx": {minWidth: "64px", height: "auto", background: "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)", boxShadow: "0 4px 12px rgba(99, 102, 241, 0.4)", transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", "&:hover": {background: "linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)", transform: "scale(1.05)", boxShadow: "0 8px 20px rgba(99, 102, 241, 0.6)"}, "&:active": {transform: "scale(0.95)", boxShadow: "0 2px 8px rgba(99, 102, 241, 0.4)"}, "&:disabled": {background: "rgba(99, 102, 241, 0.5)", transform: "none"}}}, [__jacJsx(Send, {}, [])])])])]);
}
function app() {
